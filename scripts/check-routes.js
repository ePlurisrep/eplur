...
// Directories to ignore when scanning (build output, deps, git)
const IGNORED = new Set(['node_modules', '.next', '.turbo', '.git'])

function isIgnored(p) {
  const parts = p.split(path.sep)
  return parts.some((seg) => IGNORED.has(seg))
}

function walk(dir, cb) {
  if (isIgnored(path.relative(root, dir))) return
  let entries
  try {
...
// Find all 'pages' directories in the project (excluding node_modules/.next etc.)
const pagesDirs = []
walk(root, (p, stat) => {
...
if (uniquePages.length > 1) {
  ...
  for (const d of uniquePages) console.error(' -', d)
  console.error('\nPlease keep only the repository root `pages/` folder (legacy) and move all UI into `app/`.')
  process.exit(1)
}

// Scan for forbidden files anywhere (excluding ignored dirs)
...
if (fs.existsSync(rootPages) && !isIgnored(path.relative(root, rootPages))) {
